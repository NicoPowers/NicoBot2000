...
void quadencCallback(const std_msgs::Int16MultiArray& msg){
 ...
 qe1 = msg.data[0]*0.000473211;//0.00007302800423;//0.00007356235548;//0.00007391858965;//0.00007427482381;//0.00007436388235;//fr //convert qecounts into m 
 qe2 = msg.data[1]*0.000473211;//0.00007302800423;//0.00007356235548;//0.00007391858965;//0.00007427482381;//0.00007432825894; //fl

dist = (qe1+qe2)/2;
dth = (qe1-qe2)/2;
dth = dth/0.09375; //convert to radians of rotation .14605m is 11.5in/2 (half of wheel base) [.147701 is a calibrated value]
dx = dist*cos(th);
dy = dist*sin(th);
x += dx;
y += dy;
